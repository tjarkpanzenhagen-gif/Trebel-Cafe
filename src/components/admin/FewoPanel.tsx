...
// ─── Availability View (read-only — freigeben by cancelling bookings) ─────────

function AvailabilityView({ apartmentId }: { apartmentId: string }) {
  ...
  const active = bookings.filter((b) => b.apartmentId === apartmentId && (b.status === "pending" || b.status === "confirmed"));

  if (active.length === 0) {
    return (
      <div className="py-14 text-center">
        <p className="font-playfair text-3xl text-espresso/15 mb-1">—</p>
        <p className="font-dm text-sm text-espresso/30">Keine aktiven Belegungen</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      <p className="font-dm text-xs text-espresso/40">
        Aktive Belegungen — zum Freigeben auf „Freigeben" klicken
      </p>
      {active
        .sort((a, b) => a.checkIn.localeCompare(b.checkIn))
        .map((b) => (
          <div key={b.id} className="flex items-center justify-between gap-4 border border-sand rounded-xl px-4 py-3 bg-white">
            <div className="min-w-0">
              <div className="flex items-center gap-2 mb-0.5">
                <span className={`inline-block w-1.5 h-1.5 rounded-full ${b.status === "confirmed" ? "bg-green-500" : "bg-amber-400"}`} />
                <p className="font-dm text-sm text-espresso font-medium">{b.name}</p>
              </div>
              <p className="font-dm text-xs text-espresso/50">
                {b.checkIn} → {b.checkOut} · {b.nights} Nacht{b.nights !== 1 ? "e" : ""}
              </p>
            </div>
            <div className="text-right shrink-0">
              <p className="font-dm text-xs text-espresso/30 mb-1">
...
const SECTIONS: { key: Section; label: string; icon: string }[] = [
  ...
  { key: "verfuegbarkeit", label: "Belegungen", icon: "📅" },
  ...
  return (
    <div className="grid grid-cols-[220px_1fr] gap-0 min-h-[520px] rounded-2xl border border-sand overflow-hidden bg-white">
      ...
      <div className="p-6 overflow-y-auto">
        ...
        {activeSection === "verfuegbarkeit" && <AvailabilityView key={apt.id + "-avail"} apartmentId={apt.id} />}
